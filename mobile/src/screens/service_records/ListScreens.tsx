...
export default function ListScreen({ navigation }: Props) {
  ...
  const iconByTipo = (tipo: string) => {
    if (tipo === "INSTALACION") return "plus-circle-outline";
    if (tipo === "CAMBIO") return "swap-horizontal-circle-outline";
    if (tipo === "RETIRO") return "minus-circle-outline";
    return "circle-edit-outline"; // MODIFICACION
  };
    ...
    >
      ...
      <Card.Title
        ...
        left={(props) => (
          <Avatar.Icon
            {...props}
            icon={iconByTipo(item.tipoOrden)}
            size={44}
      ...
      <Card.Content style={styles.cardContent}>
        ...
        <View style={styles.footerRow}>
          <Text variant="labelSmall" style={styles.dateText}>
            {formatDate((item as any).creadoEn ?? (item as any).createdAt)}
          </Text>
        </View>
      </Card.Content>
    </Card>
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <Appbar.Header style={{ backgroundColor: "#ffffff", elevation: 0 }}>
        ...
        <Appbar.Content
          title="Registros M9MEX"
          titleStyle={{ fontWeight: "800", color: "#111827" }}
        />
